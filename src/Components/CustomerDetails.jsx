...
const CustomerDetails = ({ customer, onBack }) => {
    ...
    const fetchCustomerPurchases = async () => {
        try {
            ...
            calculateMonthlyStats(salesData);
        } catch (error) {
    ...
    const calculateMonthlyStats = (sales) => {
        ...
        sales.forEach(sale => {
            if (sale.createdAt) {
                ...
                stats[monthKey].totalAmount += sale.totalPrice;
                stats[monthKey].totalQuantity += sale.quantity;
                
                if (!stats[monthKey].products[sale.productName]) {
                    stats[monthKey].products[sale.productName] = 0;
                }
                stats[monthKey].products[sale.productName] += sale.quantity;

                // Use actual payment mode from sale data
                if (sale.paymentMode === 'online') {
                    stats[monthKey].onlinePayments += sale.totalPrice;
                } else {
                    stats[monthKey].cashPayments += sale.totalPrice;
                }
            }
        });
                        ...
                        <div className="space-y-4">
                            {Object.entries(monthlyStats).map(([month, stats]) => (
                                <div key={month} className="border rounded p-4">
                                    <h4 className="font-semibold text-lg mb-2">{formatMonth(month)}</h4>
                                ...
                                <tbody>
                                    {purchases.map((purchase) => (
                                        <tr key={purchase.id}>
                                            ...
                                            <td>
                                                <span className={`badge ${purchase.paymentMode === 'online' ? 'badge-success' : 'badge-info'}`}>
